...
public:
    ...
    int genop(eOpEnum op,int a,int b,int c) {
        return genop(MKOP_ABC(op, a, b, c));
    }
    int genop(eOpEnum op,int a,int b) {
        return genop(MKOP_AsBx(op, a, b));
    }

    int genop(mrb_code i)
    {
        if (m_pc == m_icapa) {
            m_icapa *= 2;
            m_iseq = (mrb_code *)s_realloc(m_iseq, sizeof(mrb_code)*m_icapa);
            if (m_lines) {
                m_lines = (uint16_t*)s_realloc(m_lines, sizeof(uint16_t)*m_icapa);
                m_irep->lines = m_lines;
            }
        }
        m_iseq[m_pc] = i;
        if (m_lines) {
            m_lines[m_pc] = m_lineno;
        }
        return m_pc++;
    }
    ...
    int new_label();
    int genop_peep(mrb_code i, bool val);
    void push_();
    void pop_sp(int v=1) {
        m_sp-=v;
    }
    ...
    void scope_add_irep(mrb_irep *irep);
protected:
    void do_lambda_body(mrb_ast_node *body);
    void do_lambda_args(ArgsStore * args);
    void do_for_body(ForNode * tree);
    void do_lambda_internal(LambdaCommonNode *tree);
...
int codegen_scope::genop_peep(mrb_code i, bool val)
{
    ...
    switch (c1) {
        ...
        case OP_RETURN:
            switch (c0) {
                ...
                case OP_SETGLOBAL:
                    ...
                    genop_peep(i0, false);
                    i0 = m_iseq[m_pc-1];
                    return genop(MKOP_AB(OP_RETURN, GETARG_A(i0), OP_R_NORMAL));
#if 0
                case OP_SEND:
        ...
        case OP_JMPNOT:
            if (c0 == OP_MOVE && GETARG_A(i) == GETARG_A(i0)) {
                m_iseq[m_pc-1] = MKOP_AsBx(c1, GETARG_B(i0), GETARG_sBx(i));
                return m_pc-1;
            }
            break;
        default:
...
void codegen_scope::do_for_body(ForNode * tree)
{
...
void codegen_scope::do_lambda_internal(LambdaCommonNode *tree)
{
    m_mscope = 0;
...
{
    codegen_scope *parent = this;
    m_mscope = 1;

    if(tree->args())
        do_lambda_args(tree->args());
    do_lambda_body(tree->body());
    int res = parent->m_irep->rlen-1;
    finish();
    return res;
}
...
int codegen_scope::visit(ScopeNode *n)
{
    ...
    bool val = this->m_val_stack.back();
    ...
    if (!m_iseq) {
        ...
        if (scope->m_nregs == 0) {
            ...
            scope->genop_peep(MKOP_AB(OP_RETURN, scope->m_sp-1, OP_R_NORMAL), false);
        }
    }
    scope->finish();
    if (!m_irep) {
...
mrb_sym codegen_scope::attrsym(mrb_sym a)
{
    ...
    name2 = (char *)palloc(len+2); // '=' + '\0'
    memcpy(name2, name, len);
    name2[len] = '=';
    name2[len+1] = '\0';

    return mrb_intern(m_mrb, name2, len+1);
}
...
{
    pop_sp();
    genop(MKOP_ABC(OP_SEND, m_sp, new_msym(mrb_intern(m_mrb, "intern", 6)), 0));
    push_();
}
...
void codegen_scope::visit(UndefNode *no) {
    bool val = m_val_stack.back();
    int undef = new_msym(mrb_intern(m_mrb, "undef_method", 12));
    int num = 0;
...
void codegen_scope::visit(AliasNode *n) {
    ...
    int c = new_msym(mrb_intern(m_mrb, "alias_method", 12));
...
void codegen_scope::visit_loop(BinaryNode *n,eOpEnum op) {
    ...
    lp->pc1 = genop(MKOP_sBx(OP_JMP, 0));
    lp->pc2 = new_label();
    codegen(n->rhs(), false);
    dispatch(lp->pc1);
    codegen(n->lhs(), true);
...
void codegen_scope::visit(YieldNode *node) {
    ...
    if (sendv)
        n = CALL_MAXARGS;
    genop( MKOP_ABC(OP_SEND, m_sp, new_msym(mrb_intern(m_mrb, "call", 4)), n));
    if (val)
        push_();
}
...
void codegen_scope::visit(IfNode *node) {
    ...
    pop_sp();
    pos1 = genop_peep(MKOP_AsBx(OP_JMPNOT, m_sp, 0),val);
...
void codegen_scope::visit(OpAsgnNode *node) {
    ...
    if (len == 2 &&
        ...
        pop_sp();
        pos = genop_peep( MKOP_AsBx(name[0] == '|' ? OP_JMPIF : OP_JMPNOT, m_sp, 0),val);
        codegen( node->rhs(), true);
        pop_sp();
        gen_assignment(node->lhs(), m_sp, val);
        dispatch(pos);
...
void codegen_scope::visit(ForNode *n) {
    ...
    s->do_for_body(n);
    genop( MKOP_Abc(OP_LAMBDA, m_sp, m_irep->rlen-1, OP_L_BLOCK));
    pop_sp();
    int idx = new_msym(mrb_intern(m_mrb, "each", 4));
...
void codegen_scope::visit(RescueNode *node) {
    ...
    onerr = genop(MKOP_Bx(OP_ONERR, 0));
        ...
        while (n2) {
            ...
            do {
                if (n4) {
                    ...
                    genop(MKOP_ABx(OP_GETCONST, m_sp, new_msym(mrb_intern(m_mrb, "StandardError", 13))));
                    push_();
                }
                genop(MKOP_AB(OP_MOVE, m_sp, exc));
                pop_sp();
                genop(MKOP_ABC(OP_SEND, m_sp, new_msym(mrb_intern(m_mrb, "===", 3)), 1));
                int tmp = genop(MKOP_AsBx(OP_JMPIF, m_sp, pos2));
                pos2 = tmp;
                if (n4) {
                    n4 = n4->right();
                }
            } while (n4);
            pos1 = genop(MKOP_sBx(OP_JMP, 0));
            ...
            int tmp = genop(MKOP_sBx(OP_JMP, exend));
            exend = tmp;
            n2 = n2->right();
            push_();
        }
...
void codegen_scope::visit(CaseNode *node) {
    ...
    while (tree) {
        ...
        while (n) {
            ...
            if (head) {
                ...
                pop_sp();
                if (n->left()->getType() == NODE_SPLAT) {
                    genop(MKOP_ABC(OP_SEND, m_sp, new_msym(mrb_intern_lit(m_mrb, "__case_eqq")), 1));
                }
                else {
                    genop(MKOP_ABC(OP_SEND, m_sp, new_msym(mrb_intern_lit(m_mrb, "===")), 1));
                }
            }
            else {
                pop_sp();
        ...
        if (val)
            pop_sp();
        int tmp = genop(MKOP_sBx(OP_JMP, pos3));
        pos3 = tmp;
        if (pos1)
            dispatch(pos1);
        tree = tree->right();
    ...
    if (val) {
        ...
        pop_sp();
        genop(MKOP_AB(OP_MOVE, m_sp, pos));
        push_();
    } else if (pos3)
        dispatch_linked(pos3);
...
void codegen_scope::visit(BlockNode *node) {
    ...
    s->do_lambda_internal(node);
    genop(MKOP_Abc(OP_LAMBDA, m_sp, parent->m_irep->rlen - 1, OP_L_BLOCK));
    push_();
}
void codegen_scope::visit(LambdaNode *node) {
    ...
    genop(MKOP_Abc(OP_LAMBDA, m_sp, parent->m_irep->rlen - 1, OP_L_LAMBDA));
    push_();
}
...
void codegen_scope::visit(EnsureNode *nn) {
    ...
    m_ensure_level++;
    codegen(nn->body(), val);
    this->m_val_stack.push_back(false);
    idx = visit(nn->ensure());
    this->m_val_stack.pop_back();
...
void codegen_scope::visit(XstrNode *xn) {
    ...
    int ai = m_mrb->gc().arena_save();
    int sym = new_sym( mrb_intern(m_mrb, "Kernel", 6));
    int off = new_lit(mrb_str_new(m_mrb, p, len));
    ...
    genop( MKOP_ABx(OP_STRING, m_sp, off));
    pop_sp();
    sym = new_sym( mrb_intern(m_mrb, "`", 1));
...
void codegen_scope::visit(DregxNode *dn) {
    ...
    pop_sp();
    sym = new_sym( mrb_intern(m_mrb, "compile", 7));
    genop( MKOP_ABC(OP_SEND, m_sp, sym, argc));
    m_mrb->gc().arena_restore(ai);
    push_();
}
...
void codegen_scope::scope_add_irep(mrb_irep *irep)
{
    if (m_irep == NULL) {
        m_irep = irep;
        return;
    }
    if (m_irep->rlen == m_rcapa) {
        m_rcapa *= 2;
        m_irep->reps = (mrb_irep**)this->s_realloc(m_irep->reps, sizeof(mrb_irep*)*m_rcapa);
    }
    m_irep->reps[m_irep->rlen] = irep;
    m_irep->rlen++;
}
...
codegen_scope* codegen_scope::create(mrb_state *mrb, codegen_scope *prev, const tLocals &lv)
{
    ...
    prev->scope_add_irep(p->m_irep);
    p->m_rcapa = 8;
    p->m_irep->reps = (mrb_irep**)mrb->gc()._malloc(sizeof(mrb_irep*)*p->m_rcapa);

    p->m_icapa = 1024;
    p->m_iseq = (mrb_code*)mrb->gc()._malloc(sizeof(mrb_code)*p->m_icapa);
    p->m_irep->iseq = p->m_iseq;
    ...
    p->m_scapa = 256;
    p->m_irep->syms = (mrb_sym*)mrb->gc()._malloc(sizeof(mrb_sym)*p->m_scapa);
    p->m_irep->slen = 0;
    ...
    if (p->m_filename) {
        p->m_lines = (uint16_t *)mrb->gc()._malloc(p->m_icapa*sizeof(uint16_t));
    }
...
void codegen_scope::finish()
{
    ...
    if (m_iseq) {
        ...
        irep->ilen = m_pc;
        if (m_lines) {
            irep->lines = (uint16_t *)s_realloc(m_lines, sizeof(uint16_t)*m_pc);
        }
        else {
            irep->lines = 0;
        }
...
RProc *mrb_generate_code (mrb_state *mrb, parser_state *p) {
