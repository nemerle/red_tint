...
static mrb_bool iv_foreach(mrb_state *mrb, iv_tbl *t, iv_foreach_func *func, void *p)
{
    khash_t(iv) *h = &t->h;
    khiter_t k;
    ...
    if (h) {
        for (k = kh_begin(h); k != kh_end(h); k++) {
            if (kh_exist(h, k)){
                int n = (*func)(mrb, kh_key(h, k), kh_value(h, k), p);
                if (n > 0)
                    return false;
                if (n < 0) {
                    kh_del(iv, h, k);
...
size_t
mrb_gc_mark_iv_size(mrb_state *mrb, RObject *obj)
{
    return iv_size(mrb, obj->iv);
}

void
mrb_gc_free_iv(mrb_state *mrb, RObject *obj)
{
    if (obj->iv) {
        iv_free(mrb, obj->iv);
    }
...
void
mrb_obj_iv_ifnone(mrb_state *mrb, RObject *obj, mrb_sym sym, mrb_value v)
{
    iv_tbl *t = obj->iv;
