 ...
 *    2.0**3      #=> 8.0
 */
static mrb_value num_pow(mrb_state *mrb, mrb_value x)
{
    int both_int = false;
    mrb_float d;
...
static mrb_value num_div(mrb_state *mrb, mrb_value x)
{
    mrb_float y;
...
static void flodivmod(mrb_state *mrb, mrb_float x, mrb_float y, mrb_float *divp, mrb_float *modp)
{
    mrb_float div;
    mrb_float mod;
 ...
 *     (1.0).eql?(1.0)   #=> true
 */
static mrb_value num_eql(mrb_state *mrb, mrb_value x)
{
    mrb_bool eql_p=false;
...
{
    if (mrb_obj_equal(x, y))
        return mrb_true_value();
    return mrb->funcall(y, "==", 1, x);
}
 ...
 */
static mrb_value flo_hash(mrb_state *mrb, mrb_value num)
{
    mrb_float d;
    char *c;
    size_t i;
...
static mrb_value fix_mul(mrb_state *mrb, mrb_value x)
{
    mrb_value y = mrb->get_arg<mrb_value>();
...
static mrb_value fix_rev(mrb_state *mrb, mrb_value num)
{
...
static mrb_value fix_and(mrb_state *mrb, mrb_value x)
{
    mrb_int val;
    mrb_value y = mrb->get_arg<mrb_value>();
...
static mrb_value fix_or(mrb_state *mrb, mrb_value x)
{
    mrb_int val;
...
static mrb_value fix_xor(mrb_state *mrb, mrb_value x)
{
    mrb_int val;
...
mrb_value mrb_flo_to_fixnum(mrb_state *mrb, mrb_value x)
{
    ...
    if (x.is_float()) {
        ...
        if (isinf(d)) {
            mrb->mrb_raise(E_FLOATDOMAIN_ERROR, d < 0 ? "-Infinity" : "Infinity");
        }
        if (isnan(d)) {
            mrb->mrb_raise(E_FLOATDOMAIN_ERROR, "NaN");
        }
        z = (mrb_int)d;
    }
 ...
 * result.
 */
static mrb_value fix_plus(mrb_state *mrb, mrb_value self)
{
    mrb_value other = mrb->get_arg<mrb_value>();
 ...
 * result.
 */
static mrb_value fix_minus(mrb_state *mrb, mrb_value self)
{
    mrb_value other = mrb->get_arg<mrb_value>();
...
mrb_init_numeric(mrb_state *mrb)
{
    ...
    integer = &mrb->define_class("Integer",  numeric)
            .define_method("to_i", int_to_i, MRB_ARGS_NONE())              /* 15.2.8.3.24 */
            .define_method("to_int", int_to_i, MRB_ARGS_NONE())
    ...
    mrb->fixnum_class = &mrb->define_class("Fixnum", integer)
            ...
            .define_method("~",        fix_rev,           MRB_ARGS_NONE()) /* 15.2.8.3.8  */
            .define_method("&",        fix_and,           MRB_ARGS_REQ(1)) /* 15.2.8.3.9  */
            .define_method("|",        fix_or,            MRB_ARGS_REQ(1)) /* 15.2.8.3.10 */
            .define_method("^",        fix_xor,           MRB_ARGS_REQ(1)) /* 15.2.8.3.11 */
            .define_method("<<",       fix_lshift,        MRB_ARGS_REQ(1)) /* 15.2.8.3.12 */
    ...
    mrb->float_class = &mrb->define_class("Float", numeric)
            .undef_class_method("new")
            .define_method("+",         flo_plus,         MRB_ARGS_REQ(1)) /* 15.2.9.3.1  */
            .define_method("-",         flo_minus,        MRB_ARGS_REQ(1)) /* 15.2.9.3.2  */
            .define_method("*",         flo_mul,          MRB_ARGS_REQ(1)) /* 15.2.9.3.3  */
            .define_method("%",         flo_mod,          MRB_ARGS_REQ(1)) /* 15.2.9.3.5  */
            .define_method("==",        flo_eq,           MRB_ARGS_REQ(1)) /* 15.2.9.3.7  */
            .define_method("ceil",      flo_ceil,         MRB_ARGS_NONE()) /* 15.2.9.3.8  */
            .define_method("finite?",   flo_finite_p,     MRB_ARGS_NONE()) /* 15.2.9.3.9  */
            .define_method("floor",     flo_floor,        MRB_ARGS_NONE()) /* 15.2.9.3.10 */
            .define_method("infinite?", flo_infinite_p,   MRB_ARGS_NONE()) /* 15.2.9.3.11 */
            .define_method("round",     flo_round,        MRB_ARGS_NONE()) /* 15.2.9.3.12 */
            .define_method("to_f",      flo_to_f,         MRB_ARGS_NONE()) /* 15.2.9.3.13 */
