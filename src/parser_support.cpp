...
void mrb_parser_state::mrb_parser_set_filename(char const* f)
{
    mrb_sym const sym = m_mrb->intern_cstr(f);
    size_t len;
    m_filename = mrb_sym2name_len(this->m_mrb, sym, len);
    m_lineno = (filename_table_length > 0)? 0 : 1;
    for(size_t i = 0; i < this->filename_table_length; ++i) {
        if(this->filename_table[i] == sym) {
            this->current_filename_index = i;
            return;
        }
    }

    this->current_filename_index = this->filename_table_length++;

    mrb_sym* const new_table = (mrb_sym*)this->parser_palloc(sizeof(mrb_sym) * this->filename_table_length);
    if (this->filename_table) {
        memcpy(new_table, this->filename_table, sizeof(mrb_sym) * this->filename_table_length);
    }
    this->filename_table = new_table;
    this->filename_table[this->filename_table_length - 1] = sym;
}
char const* mrb_parser_state::mrb_parser_get_filename(uint16_t idx) {
    if (idx >= filename_table_length) { return NULL; }
    else {
        size_t len;
        return mrb_sym2name_len(m_mrb, filename_table[idx], len);
    }
}
