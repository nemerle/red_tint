...
mrb_debug_info_alloc(mrb_state *mrb, mrb_irep *irep)
{
    ...
    ret = (mrb_irep_debug_info *)mrb->gc()._malloc(sizeof(*ret));
    *ret = initial;
    irep->debug_info = ret;
    return ret;
}
                           ...
                           uint32_t start_pos, uint32_t end_pos)
{
    ...
    ret = (mrb_irep_debug_info_file *)mrb->gc()._malloc(sizeof(*ret));
    info->files =
            (mrb_irep_debug_info_file**)(
                info->files
                ?mrb->gc()._realloc(info->files, sizeof(mrb_irep_debug_info_file*) * (info->flen + 1))
               : mrb->gc()._malloc(sizeof(mrb_irep_debug_info_file*)));
    ...
    ret->filename_sym = mrb->intern2(irep->filename, fn_len);
    len = 0;
    ...
    case mrb_debug_line_ary:
        ...
        ret->lines.ary = (uint16_t *)mrb->gc()._malloc(file_pc_count*sizeof(uint16_t));
    ...
    case mrb_debug_line_flat_map: {
        ...
        ret->lines.flat_map = mrb->gc().new_t<mrb_irep_debug_info_line>();
        ret->line_entry_count = 0;
        for(i = 0; i < file_pc_count; ++i) {
            if(irep->lines[start_pos + i] == prev_line) { continue; }

            ret->lines.flat_map = (mrb_irep_debug_info_line*)mrb->gc()._realloc(
                        ret->lines.flat_map,
