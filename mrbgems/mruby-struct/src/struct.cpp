...
make_struct(mrb_state *mrb, mrb_value name, mrb_value members, struct RClass * klass)
{
    ...
    for (i=0; i< len; i++) {
        ...
        if (mrb_is_local_id(id) || mrb_is_const_id(id)) {
            if (i < N_REF_FUNC) {
                c->define_method_id(mrb, id, ref_func[i], ARGS_NONE());
            }
            else {
                c->define_method_id(mrb, id, mrb_struct_ref, ARGS_NONE());
            }
            c->define_method_id(mrb, mrb_id_attrset(mrb, id), mrb_struct_set_m, ARGS_REQ(1));
        }
    }
    return nstr;
}
...
mrb_mruby_struct_gem_init(mrb_state* mrb)
{
    mrb->define_class("Struct",  mrb->object_class)
            .define_class_method(mrb, "new",       mrb_struct_s_def,       ARGS_ANY())  /* 15.2.18.3.1  */
            .define_method(mrb, "==",              mrb_struct_equal,       ARGS_REQ(1)) /* 15.2.18.4.1  */
            .define_method(mrb, "[]",              mrb_struct_aref,        ARGS_REQ(1)) /* 15.2.18.4.2  */
            .define_method(mrb, "[]=",             mrb_struct_aset,        ARGS_REQ(2)) /* 15.2.18.4.3  */
            .define_method(mrb, "members",         mrb_struct_members_m,   ARGS_NONE()) /* 15.2.18.4.6  */
            .define_method(mrb, "initialize",      mrb_struct_initialize_m,ARGS_ANY())  /* 15.2.18.4.8  */
            .define_method(mrb, "initialize_copy", mrb_struct_init_copy,   ARGS_REQ(1)) /* 15.2.18.4.9  */
            .define_method(mrb, "inspect",         mrb_struct_inspect,     ARGS_NONE()) /* 15.2.18.4.10(x)  */
            .define_alias(mrb, "to_s", "inspect")                                       /* 15.2.18.4.11(x)  */
            .define_method(mrb, "eql?",            mrb_struct_eql,         ARGS_REQ(1)) /* 15.2.18.4.12(x)  */
            ;
}
